...
为了对比基线效果，我们从熵简科技实际业务中抽象出了四类典型的金融领域典型数据集和两类典型的通用领域数据集，
包括句子级和篇章级任务。在此基础上，我们将 FinBERT 与 Google 原生中文 BERT、哈工大讯飞实验室
开源的 BERT-wwm 和 RoBERTa-wwm-ext 这三类在中文领域应用广泛的模型进行了下游任务的对比测试。在实
验中，为了保持测试的公平性，我们没有进一步优化最佳学习率，对于四个模型均直接使用了 BERT-wwm 的最佳学习率：2e-5。
...
--------------  | ---- | :------: | :-------------: | :-----:
金融短讯类型分类  | 0.867（0.874） | 0.867（0.877） | 0.877（0.885） | **0.896（0.898）**
...
TASK\MODEL      | BERT | BERT-wwm | RoBERTa-wwm-ext | FinBERT 
--------------  | ---- | :------: | :-------------: | :-----:
金融短讯行业分类  | 0.939（0.942） | 0.932（0.942） | 0.938（0.942） | **0.953（0.955）**
...
TASK\MODEL      | BERT | BERT-wwm | RoBERTa-wwm-ext | FinBERT 
--------------  | ---- | :------: | :-------------: | :-----:
金融情绪分类  | 0.862（0.866） | 0.850（0.860） | 0.867（0.867） | **0.896（0.900）**
...
人物名称实体识别      | 0.887 | 0.887 | 0.891 | **0.941**

## 3.5. 实验五：酒店评论信息分类
### 3.5.1. 实验任务

此任务来源于 **[携程网](https://www.ctrip.com/)**
，典型的通用领域评论信息、单类别评论二分类任务，其核心任务为判定评论信息的正负极性，原始数据为酒店评论信息。
...
TASK\MODEL      | BERT | BERT-wwm | RoBERTa-wwm-ext | FinBERT 
--------------  | :----: | :------: | :-------------: | :-----:
酒店评论分类      | 0.892(0.895) | 0.892(0.894) | 0.897(0.899) | 0.893(0.894)

## 3.6. 实验六：多类别评论信息分类
### 3.6.1. 实验任务

此任务来源于 **[中文情感分析语料库](https://download.csdn.net/download/weixin_38395744/10231401)**
，典型的通用领域评论信息、多类别评论二分类任务，其核心任务为判定评论信息的正负极性，原始数据共10 个类别，包括
书籍、平板、手机、水果、洗发水、热水器、蒙牛、衣服、计算机、酒店。

### 3.6.2. 数据集

该任务原始数据集共 包括6 万+条评论数据，其中正、负向评论各约 3 万条，实验随机筛选训练集样本1000条，
测试集样本5000条，训练集和测试集正负评论分布如下：
...
TASK\MODEL      | BERT | BERT-wwm | RoBERTa-wwm-ext | FinBERT 
--------------  | :----: | :------: | :-------------: | :-----:
多类别评论信息分类      | 0.925(0.929) | 0.918(0.920) | 0.927(0.928) | 0.926(0.928)

## 3.7. 总结

在本次基线测试中，我们以金融场景中所遇到四类实际业务问题和数据入手进行对比实验，包括金融类短讯类型分类任务、金融文本行业分类、
金融情绪分析任务以及金融类实体识别任务，同时选取两类通用领域语料任务，对比 FinBERT 和 Google 原生中文BERT、 
BERT-wwm、RoBERTa-wwm-ext 这三种通用领域的预训练模型

可知，**FinBERT** 效果提升显著，在领域内任务 F1-score 上平均可以提升 **2~5.7** 个百分点，同时可以保持在通用领域内任务性能基本保持不变。
# 4. 结语
